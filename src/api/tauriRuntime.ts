...
export async function invokeTauri<T>(command: string, payload?: Record<string, unknown>) {
  const tauriCore = await loadTauriCore()
  if (tauriCore?.invoke) {
    return tauriCore.invoke<T>(command, payload)
  }

  if (hasLegacyTauriInvoker()) {
    return window.__TAURI_INTERNALS__!.invoke<T>(command, payload)
  }
